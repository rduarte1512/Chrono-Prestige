...
module.exports = function handler(req, res) {
  ...
  const events = `<script>
    document.addEventListener("click", function (event) {
      ...
      if (!link || !window.whop) return;
      const href = link.href;
      if (href.includes("/products/")) {
        whop.track("view_content");
      } else if (href.includes("/collections/") || href === "https://chrono-prestige-nmh8jm3s.myshopify.com/") {
        whop.track("shop_click");
      }
    });
  </script>`;

  let output = html.includes(snippet)
    ? html
    : html.replace('</head>', `${snippet}\n</head>`);

  if (!output.includes('whop.track("shop_click")')) {
